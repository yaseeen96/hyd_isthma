...
class ProgramListResource extends JsonResource
{
    ...
    public function toArray(Request $request): array
    {
        ...
        $data = [
            ...
            'datetime' => !empty($this->from_time) && !empty($this->from_time) ? ( date('Y-m-d', strtotime($this->date)) . ' ' . Carbon::parse($this->from_time)->format('h:i A'). ' - ' . Carbon::parse($this->to_time)->format('h:i A') ) : null,
