...
class SessionThemeListResource extends JsonResource
{
    ...
    public function toArray(Request $request): array
    {
        ...
        return [
            ...
            'session_convener' => !empty($this->convener) ? ( empty($lang) ? $this->convener : ($speaker_details ? $speaker_details->{$lang . '_name'} : '') ) : '',
            'convener_bio' => !empty($this->convener) ? (empty($lang) ? '' : ($speaker_details->bio ? $speaker_details->{$lang . '_bio'} : '') ) : '',
            'convener_image' => !empty($speaker_details) && !empty($this->convener) ? ($speaker_details->getMedia('speaker_image')->first() ? $speaker_details->getMedia('speaker_image')->first()->getUrl() : env('APP_URL').'/assets/img/no-image.png') : null,
