...
check_dependencies() {
  ...
  # Check objdump / llvm-objdump
  if command -v objdump >/dev/null 2>&1; then
    OBJDUMP_CMD="objdump"
    available_tools+=("objdump")
    print_status "success" "objdump" "$(which objdump)"
  elif command -v llvm-objdump >/dev/null 2>&1; then
    OBJDUMP_CMD="llvm-objdump"
    available_tools+=("llvm-objdump")
    print_status "success" "llvm-objdump" "$(which llvm-objdump)"
  else
    # Try finding llvm-objdump in Android NDK directories
    local ndk_paths=()
    if [[ -n "$ANDROID_NDK_HOME" ]]; then ndk_paths+=("$ANDROID_NDK_HOME"); fi
    if [[ -n "$ANDROID_HOME" ]]; then ndk_paths+=("$ANDROID_HOME/ndk"); fi
    if [[ -n "$ANDROID_SDK_ROOT" ]]; then ndk_paths+=("$ANDROID_SDK_ROOT/ndk"); fi
    ndk_paths+=(
    ...
    for ndk_dir in "${ndk_paths[@]}"; do
      if [[ -d "$ndk_dir" ]]; then
        local found_objdump=$(find "$ndk_dir" -name "llvm-objdump*" -type f 2>/dev/null | head -n 1)
        if [[ -n "$found_objdump" ]]; then
          OBJDUMP_CMD="$found_objdump"
          break
        fi
      fi
    done
    ...
    if [[ -n "$OBJDUMP_CMD" ]]; then
      available_tools+=("objdump (NDK)")
      print_status "success" "objdump (Android NDK)" "$OBJDUMP_CMD"
    else
      missing_tools+=("objdump")
      print_status "error" "objdump / llvm-objdump not found"
    fi
  fi
...
for match in $matches; do
  ...
  res="$("$OBJDUMP_CMD" -p "${match}" 2>/dev/null | grep LOAD | awk '{ print $NF }' | head -1)"
