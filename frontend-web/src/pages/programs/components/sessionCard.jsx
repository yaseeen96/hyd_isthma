...
import { FiChevronDown, FiChevronUp, FiClock, FiMapPin, FiUser } from 'react-icons/fi';

const SessionCard = forwardRef(({ session, index, expandedSessions, toggleSession, openModal, backgroundColor, programColor }, ref) => {
            ...
            {isExpanded && (
                <div className="mt-4">
                    {session.programs.length > 0 ? (
                        session.programs.map((program) => (
                            <div key={program.id} className={`mt-4 p-4 border rounded-lg flex ${programColor} shadow-sm`}>
                                ...
                                <div className="flex-grow">
                                    ...
                                    <div className="flex items-center space-x-2 text-gray-600 mt-1">
                                        <FiUser className="text-primary-500" size={35} />
                        ...
                        <p className="mt-4">No programs available for this session.</p>
                    )}
                </div>
            )}
        </div>
